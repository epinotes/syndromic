get_agesex_sro <- function(username, password, 
  ...
  resultmissing <- result_site_ageSex %>%
    ...
    group_by(site, timeresolution, age_ten_year) %>%
  ...
  result_site_ageSex %>%
    group_by(site,  timeresolution, sex,   age_ten_year) %>%
